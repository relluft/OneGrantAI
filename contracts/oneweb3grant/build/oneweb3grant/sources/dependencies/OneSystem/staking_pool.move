...
public(package) fun request_withdraw_stake(
    ...
    let total_oct_withdraw_amount = principal_withdraw_amount + rewards_withdraw.value();

    pool.pending_total_sui_withdraw = pool.pending_total_sui_withdraw + total_oct_withdraw_amount;
...
fun calculate_fungible_staked_oct_withdraw_amount(
    ...
    let total_oct_amount = latest_exchange_rate.get_sui_amount(
        fungible_staked_oct_data_total_supply,
    );

    // min with total_oct_amount to prevent underflow
    ...
    let total_rewards = total_oct_amount - fungible_staked_oct_data_principal_amount;
...
fun withdraw_rewards(
    ...
    let total_oct_withdraw_amount = exchange_rate.get_sui_amount(pool_token_withdraw_amount);
    let mut reward_withdraw_amount = if (total_oct_withdraw_amount >= principal_withdraw_amount) {
        total_oct_withdraw_amount - principal_withdraw_amount
    } else 0;
...
public(package) fun calculate_rewards(
    ...
    let total_oct_withdraw_amount = new_epoch_exchange_rate.get_sui_amount(
        pool_token_withdraw_amount,
    );

    let mut reward_withdraw_amount = if (total_oct_withdraw_amount >= staked_amount) {
        total_oct_withdraw_amount - staked_amount
    } else 0;
    reward_withdraw_amount = reward_withdraw_amount.min(pool.rewards_pool.value());
...
fun test_calculate_fungible_staked_oct_withdraw_amount(
    ...
    total_oct_amount = total_oct_amount.max(1);
    ...
    let total_pool_token_amount = mul_div!(total_oct_amount, pool_token_frac as u64, 10_000).max(1);

    let exchange_rate = PoolTokenExchangeRate {
        sui_amount: total_oct_amount,
